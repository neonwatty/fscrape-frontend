...
import type { ForumPost, PlatformStats, PostFilters, TimeSeriesData, AuthorStats } from '@/lib/db/types'
import { 
  getDatabaseSummary, 
  getPosts,
  getRecentPosts,
...
export function DatabaseProvider({ 
  ...
  // Query cache
  const [queryCache, setQueryCache] = useState<Map<CacheKey, CacheEntry<any>>>(new Map())
  ...
  const getCachedOrQuery = useCallback(<T,>(
    key: CacheKey,
    queryFn: () => T,
    ttl: number = cacheTTL
  ): T => {
  ...
  const loadDatabaseFn = useCallback(async (pathOrData?: string | Uint8Array) => {
    setIsLoading(true)
    setError(null)
  ...
  // Query functions with caching
  const queryPosts = useCallback((filters?: PostFilters): ForumPost[] => {
    if (!isDatabaseInitialized()) return []
    const key: CacheKey = `posts_${JSON.stringify(filters || {})}`
    return getCachedOrQuery(key, () => getPosts(filters))
  }, [getCachedOrQuery])
  
  const queryRecentPosts = useCallback((limit: number = 10): ForumPost[] => {
    if (!isDatabaseInitialized()) return []
  ...
  const queryTimeSeries = useCallback((days: number = 30): TimeSeriesData[] => {
    if (!isDatabaseInitialized()) return []
    const key: CacheKey = `timeSeries_${days}`
    return getCachedOrQuery(key, () => getPostsTimeSeries(days))
  }, [getCachedOrQuery])
  
  const queryPostsByHour = useCallback((days: number = 7): ReturnType<typeof getPostsByHour> => {
    if (!isDatabaseInitialized()) return []
    const key: CacheKey = `postsByHour_${days}`
    return getCachedOrQuery(key, () => getPostsByHour(days))
  }, [getCachedOrQuery])
  
  const queryTopAuthors = useCallback((limit: number = 10): AuthorStats[] => {
    if (!isDatabaseInitialized()) return []
    return getCachedOrQuery('topAuthors', () => getTopAuthors(limit))
  }, [getCachedOrQuery])
  
  const queryTopSources = useCallback((limit: number = 10): ReturnType<typeof getTopSources> => {
    if (!isDatabaseInitialized()) return []
    return getCachedOrQuery('topSources', () => getTopSources(limit))
  }, [getCachedOrQuery])
  
  const queryScrapingSessions = useCallback((limit: number = 20): ReturnType<typeof getScrapingSessions> => {
    if (!isDatabaseInitialized()) return []
  ...
  const queryPostingHeatmap = useCallback((days: number = 30): ReturnType<typeof getPostingHeatmap> => {
    if (!isDatabaseInitialized()) return []
    const key: CacheKey = `heatmap_${days}`
    return getCachedOrQuery(key, () => getPostingHeatmap(days))
  }, [getCachedOrQuery])
  
  const queryTrendingPosts = useCallback((limit: number = 10): ForumPost[] => {
    if (!isDatabaseInitialized()) return []
    return getCachedOrQuery('trendingPosts', () => getTrendingPosts(limit))
  }, [getCachedOrQuery])
  
  const queryEngagementMetrics = useCallback((days: number = 30): ReturnType<typeof getEngagementMetrics> => {
    if (!isDatabaseInitialized()) return []
    return getCachedOrQuery('engagementMetrics', () => getEngagementMetrics(days))
  }, [getCachedOrQuery])
  
  const searchPostsQuery = useCallback((query: string, limit: number = 50): ForumPost[] => {
    if (!isDatabaseInitialized()) return []
